...
public:
    ...
    void respSave(char* mess, int playerId, int length) {
        Element element;
        char outMessage[32];
        outMessage[0] = mess[0];
        outMessage[1] = playerId;
        memcpy(&outMessage[2], mess + 1, length);
        element.message = outMessage;
        element.id = playerId;
        element.resp = true;
        push(element);
...
//        printf("%s\n",message);
        switch (message[0]) {
            ...
            case 'd':
                ...
                queue.saveToQueue(dlaSpajkiego, playerId-'0', 30);
                ...
                queue.respSave(dlaSpajkiego, playerId, 30);
