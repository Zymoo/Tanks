...
// char message should be string change
struct Element {
  int id;
  char message[30];
};
...
void clientMain(int port,
  ...
  h.onMessage([&queue](uWS::WebSocket<uWS::SERVER>* ws, char* message,
    ...
    Element element;
    strcpy(element.message, outMessage);
    element.id = playerId;
    queue.push(element);
  });
  h.listen(port);
  ...
  while (1) {
    ...
    if (!queue.empty()) {
      ...
      for (uWS::WebSocket<uWS::SERVER>* ws : websockets) {
        ...
        ws->send(tmp.message, strlen(tmp.message), uWS::OpCode::TEXT);
      }
    }
  }
}
  ...
  h.onConnection(
      [&ports, &queue, &freePlayerId, &welcomeMessage, &threadVector,
       &websockets](uWS::WebSocket<uWS::SERVER>* ws, uWS::HttpRequest req) {
        ...
        message[6] = ',';
        strcpy(message + 7, tmpWelcomeMessage.c_str());
