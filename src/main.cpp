...
#include <condition_variable>
#include <iostream>
// idk why this is global this should be in class Queue
// change
std::mutex mutexTur;
std::mutex mutex;
...
void clientMain(int port,
    Queue& queue, Queue& turQueue,
    UserMetaData* userMetaData,
    std::vector<uWS::WebSocket<uWS::SERVER>*>& websockets) {
    uWS::Hub h;
    h.onConnection([port, &queue,&turQueue, userMetaData, &websockets](
    ...
    h.onMessage([&queue, &turQueue](uWS::WebSocket<uWS::SERVER>* ws, char* message,
        ...
        case 't':
            ...
            // handle messege
            saveToQueue(element, turQueue, message, playerId, length);
            break;
        case 's':
            // shot
...
int main() {
    ...
    Queue turqueue = Queue();

    threadVector.push_back(
        std::thread(broadcast, std::ref(websockets), std::ref(turqueue)));
    uWS::Hub h;
